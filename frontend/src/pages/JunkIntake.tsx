...
const JunkIntake = () => {
  ...
  const [filterImpact, setFilterImpact] =
...
setTotalPenaltyToday(penalty);
  ...
  const fetchJunkLimits = async () => {
    try {
      const res = await fetch("http://localhost:8000/user/junk-limits", {
        headers: {
  ...
  const todaysJunk = loggedJunk.filter((l) =>
    l.timestamp.startsWith(today)
  );

  const categories = [
    ...new Set(JUNK_MENU.foods.map((f) => f.category)),
  ];

  const filteredFoods =
    filterImpact === "all"
      ? JUNK_MENU.foods
      : JUNK_MENU.foods.filter(
          (f) => getHealthImpact(f.harm_level) === filterImpact
        );
...
todaysJunk.forEach((log) => {
  ...
  todayLevelCount[item.harm_level] =
    (todayLevelCount[item.harm_level] || 0) + log.quantity;
});
  /* -------- UI -------- */
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-muted-foreground">
              Log what you ate today. Penalty applies only after allowance.
            </p>
          </div>
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              {todaysJunk.length > 0 && (
                <button
                  ...
                  className="px-3 py-1 text-sm bg-destructive/10 text-destructive rounded-lg"
                >
                ...
                {todaysJunk.map((log) => {
                  ...
                  return (
                    <motion.div
                      ...
                      exit={{ opacity: 0, x: 10 }}
                      className="flex justify-between items-center p-3 mb-2 border rounded-xl"
                    >
                      <div>
                  ...
                  {filteredFoods
                    ...
                    .map((item) => {
                      ...
                      return (
                        <div
                          key={item.id}
                          className={`p-4 rounded-xl border ${impactColors[impact]}`}
                        >
                          <p className="font-semibold">{item.name}</p>
                          <div className="text-xs text-muted-foreground mb-2 space-y-1">
                            {item.allowed_frequency && (
                              <p className="font-medium text-foreground/80">
                                Allowed: {item.allowed_frequency}
                              </p>
                            )}
                          ...
                          <div className="flex gap-2">
                            ...
                            />
                            <button
                              onClick={() => handleAddJunk(item.id)}
                              className="flex-1 bg-destructive/10 text-destructive rounded-lg"
                            >
        ...
        <div className="lg:col-span-1">
          <div className="card-elevated p-6">
            ...
            <p className="text-4xl font-bold text-destructive">
              -{totalPenaltyToday} pts
            </p>

            <p className="text-xs text-muted-foreground mt-2">
              Penalty applies only after allowance per danger level.
            </p>
          </div>
...
<div className="card-elevated p-6 mt-4">
  ...
  <p className="text-xs text-muted-foreground mt-4">
    Items of the same impact level share allowance.
  </p>
</div>
